# -*- coding: utf-8 -*-
"""Gitea RPC service for OpenMediaVault."""
...
class ServiceGitea(rpc.Service):
    """RPC service to manage the Gitea Docker stack."""
    ...
    def restart(self) -> Dict[str, str]:
        """Restart the Gitea service container."""
        _run_command(
            [
                "docker",
                "compose",
                "-f",
                "/srv/dev-disk-by-label-data/gitea/docker-compose.yml",
                "--env-file",
                "/srv/dev-disk-by-label-data/gitea/.env",
                "restart",
                "gitea-server",
            ]
        )
